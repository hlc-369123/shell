...
if [[ -n ${TOKEN} ]]; then
  ...
  if ! xms-cli $CLI host list &>/dev/null; then warnlog "Incorrect username or password information..." && exit 1; fi
else
...
get_cluster_ips(){
  ...
  rm -f /tmp/operations/{*_ip,db_list,*.log,net/*.log,net/*_ip}
...
#检查ceph集群状态
ceph_status(){
  local ceph_status=$(ceph health 2>/dev/null)
  if [[ "${ceph_status}" != HEALTH_OK ]];then
    ceph health >>${LOG_INFO}_warn.log
  fi
}

#获取osd使用率
osd_use() {
  ceph osd df 2>/dev/null| sort -n -k 7 | awk '{print $1,$7}' | grep "^[0-9]" | awk '{if ($2 >= '$CHECK_OSD_USAGE') print "使用率大于'$CHECK_OSD_USAGE'%的osd:\t" "\033[1;40;31m""osd_ID: osd."$1"\tUSE: "$2"%""\033[0m"}' | tee -a ${LOG_INFO}_warn.log
}
...
check_self_login() {
  ...
  $SSH root@${local_admin_ip} &>/dev/null hostname
  if [[ $? -eq 0 && -z ${local_action} ]]; then
    self_login="true"
    echo ${self_login}
  elif [[ -n ${local_action} ]] && ! ip addr | grep ${local_admin_ip} &>/dev/null; then
    $SSH root@${local_admin_ip} "rm -f /tmp/operations/{*_ip,db_list,*.log,net/*};mkdir -p ${NETWORK_DIR}" 2>/dev/null
  fi
}
...
distribution_of_ip() {
  ...
  for i in $(cat ${admin_ip}); do
    ...
    if [[ "$EXITVALUE" -ne 0 || "$origin" == 'Custom_IP'|| "$action" == 'get_result' ]]; then
      ...
      if [[ ${flag} == 'true' && ${action} == 'scp' ]]; then
        ...
        log "${i} CheckNetwork..."
        $SSH root@${i} "bash ${NETWORK_DIR}check_network.sh &>/dev/null &" 2>/dev/null
      elif [[ ${flag} == 'true' && ${action} == 'resource' ]]; then
        log "${i} CheckResource..."
        $SSH root@${i} "bash ${NETWORK_DIR}check_resource.sh &>/dev/null &" 2>/dev/null
      elif [[ ${flag} == 'true' && ${action} == 'get_result' ]]; then
        log "${i} GetResult..."
        $SSH root@${i} "bash ${NETWORK_DIR}get_result.sh" 2>/dev/null
      elif [[ ${flag} == 'true' && ${action} == 'clean_env' ]]; then
        if [[ -f "${ADMIN_IP}" && -s "${ADMIN_IP}" ]];then
          log "${i} CleanCheckInfo..."
          $SSH root@${i} "rm -f /tmp/operations/{*_ip,db_list,*.log,net/*}" 2>/dev/null
        else
          warnlog "No file ${ADMIN_IP} was found"
        fi
      elif [[ ${flag} == 'true' && ${action} == 'clockdiff' ]]; then
        time_sync_stat=$($SSH root@${i} 2>/dev/null "timedatectl status|awk '/synchronized/'" | awk '{print $3}')
        if [[ ${time_sync_stat} == "yes" ]]; then
          echo "时间已同步" >> ${LOG_INFO}.log
        else
          time_sync_local ${i} | tee | awk '{if ($NF >= '$CHECK_TIME_VALUE') print '$i' " 时间和本节点差为 " $NF"ms"}' >>${LOG_INFO}_warn.log
        fi
      fi
    fi
  done
...
PS3="请选择要执行得选项序列数字:=>"
num=1
select choice in 检查网络 检查资源 自定义节点检查网络和资源 清理检查后文件 退出; do
  case $choice in
  检查网络)
    get_cluster_ips
