...
refresh_the_role() {
  ...
  for ip in $host_ips; do
    ...
    if [ $xmsd_status == 'active' ]; then
      ...
      node_id=$(xms_cli_use "host list" | grep $ip | awk '{print $2}')
      node_roles=$(xms_cli_use "host show $node_id" | grep "roles" | awk '{print $4}')
      xms_cli_use "host set --roles $node_roles $node_id"
    else
...
action_es() {
  local action=$1
  es_action set ${action}
  for i in {0..9}; do
    es_stat=$(es_action show | grep elasticsearch_enabled | awk '{print $4}')
    true_num=$(xms-manage db trigger list 2>/dev/null | egrep -v "${action}|TableName" | grep [a-z] | wc -l)
...
if [ "$1" == "-h" ]; then
  ...
  if [ "$1" == "-h" ]; then
    ...
    if ! xms_cli_use "host list" &>/dev/null; then echo 'Wrong information...' && exit 1; fi
  fi
